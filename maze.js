var Maze = (function () 
{
  ...
  class Maze 
  {
    ...
    _coordForDirection(coord, direction) 
    {
      ...
      if (nextCoord[0] < 0 || nextCoord[0] >= MAZE_SIZE || nextCoord[1] < 0 || nextCoord[1] >= MAZE_SIZE) return false;
      {
          return nextCoord;
      }
    }
    ...
    breakRandomWall(maze, coord, seen) 
    {
      var y = coord[1];
      var x = coord[0];
      var walls = maze[y][x];
      if (walls > 0) 
      {
        ...
        for (let wall of directions) 
        {
          ...
          if (seen[this._hashKeyForCoord(otherWall)]) 
          {
            continue;
          }
